...
import { ImageSegmenter, ImageSegmenterResult, FilesetResolver, MPMask } from "@mediapipe/tasks-vision";
...
const CanvasGenerator = ({ userCapture, canvas, onProcessingComplete }) => {
    ...
    const backgrounds = [
        { id: 1, imageUrl: '/assets/images/BG1_City.png' },
        { id: 2, imageUrl: '/assets/images/BG2_University.png' },
        { id: 3, imageUrl: '/assets/images/BG3_CityWindow.png' },
    ];

    const footerOverlayUrl = '/assets/images/overlay-footer-x.png';
    ...
    const CalculatePercentile = (arrayValues, percentile)=>{
        ...
        if(nPercent === 1) {
            ...
            return arrayValues[intNPercent - 1] + d * (arrayValues[intNPercent] - arrayValues[intNPercent - 1])
        }
    }
    ...
    const ColorCorrectSelfie = (context)=> {
        ...
        if(loadedImages)
        {
            ...
            context.drawImage(loadedImages[selfieIdx], 0, 0, loadedImages[selfieIdx].width, loadedImages[selfieIdx].height);

            const selfieImageData = context.getImageData(0,0, loadedImages[selfieIdx].width, loadedImages[selfieIdx].height);
            ...
            if(bUseMediaPipe)
                BlurAlphaChannel(selfieImageData);
    ...
    const AddBottomImageElement = (context) => {
        ...
        {
            let aspectRatio = loadedImages[footerIdx].width / loadedImages[footerIdx].height;
            ...
            context.drawImage(loadedImages[footerIdx], leftPos, topPos, canvas.width, (canvas.height / aspectRatio)); // I'm testing this until output is 1080 x 1080
        }
    }
    //#endregion
    ...
    useEffect(()=>{
        if((bgRemovedSelfieImage && !bUseMediaPipe) || (selfieMaskReady && bgMaskVals)) 
        {
            // there's selfie and processed image is ready
            /** @type {string} */
            let selfieURL = !bUseMediaPipe && bgRemovedSelfieImage ? bgRemovedSelfieImage: userCapture;
